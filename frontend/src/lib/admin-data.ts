...
export async function getDashboardStats(range: string = "week") {
  ...
  // First get settings to determine threshold
  const settings = await Settings.findOne().select("lowStockThreshold").lean();
  const threshold = settings?.lowStockThreshold || 10;

  // Execute all other queries in parallel for better performance
  const [
    productsCount,
    customersCount,
    metrics,
    ...
    // 4. Stock alerts - parallel execution
    Product.find({
